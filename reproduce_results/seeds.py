...
seeds = pd.read_csv(pth.concatenate_path_os_independent('seeds_dataset.txt'), sep='\t')
...
concatenated.to_excel('concatenated_seeds.xlsx')
...
for miss in missing:
    for k in ks:
        ...
        ind += 1
concatenated = pd.concat(dfs)
concatenated.to_excel('concatenated_seeds_m.xlsx')
