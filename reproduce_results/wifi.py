...
wifi = pd.read_csv(pth.concatenate_path_os_independent('wifi_localization.txt'), header=None, sep='\t')
...
concatenated.to_excel('concatenated_wifi.xlsx')
...
for miss in missing:
    for k in ks:
        ...
        ind += 1
concatenated = pd.concat(dfs)
concatenated.to_excel('concatenated_wifi_m.xlsx')
